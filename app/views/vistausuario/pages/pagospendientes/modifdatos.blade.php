                        ...
                        <section class="col-lg-7 connectedSortable">                            
                                ...
                                <div class="panel panel-default">
                                  ...
                                  <div class="panel-heading">
                                    <h3 class="panel-title">Modificar datos de pago pendiente</h3>
                                  </div>
											@if ($errors->has())
                                  ...
                                  <div class="panel-body">
                                    {{ Form::open(array('url' => 'vistausuario/pagospendguardarmodifdatos', 'files' => true)) }}
										{{ Form::hidden('pagopid', $pagop->id) }}
                                        <div class="form-group">
                                                {{ Form::label('metodopago', 'Metodo de pago') }}
												@if ($errors->has())
													{{ Form::text('metodopago',Input::old('metodopago'), array( 'placeholder' => '',  'class' => 'form-control')) }}
												@else
													{{ Form::text('metodopago',$pagop->metodoPago, array( 'placeholder' => '',  'class' => 'form-control')) }}
												@endif
                                        </div>
                                        <div class="form-group">
                                                {{ Form::label('referenciapago', 'Referencia del pago') }}
												@if ($errors->has())
													{{ Form::text('referenciapago',Input::old('referenciapago'), array( 'placeholder' => '',  'class' => 'form-control')) }}
												@else
													{{ Form::text('referenciapago',$pagop->referenciaPago, array( 'placeholder' => '',  'class' => 'form-control')) }}
												@endif
                                        </div>
                                        <hr>
                                        <div class="form-group">
                                                <center>{{ Form::submit('Modificar datos de pago', array('class' => 'btn btn-success')) }}</center>
                                        </div>
